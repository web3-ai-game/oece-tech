...
export default function ArticlePage({ params }: Props) {
  ...
  const contentKey = `${article.slug}-content` as keyof typeof t;
  ...
  return (
    <article className="max-w-4xl mx-auto">
      ...
      <header className="space-y-4 mb-8">
        <Badge variant={article.category === 'Tech' ? 'default' : 'secondary'} className={`w-fit ${article.category === 'Travel' ? 'bg-accent text-accent-foreground' : ''}`}>
          {article.category === 'Tech' ? t.tech : t.travel}
        </Badge>
        <h1 className="text-4xl md:text-5xl font-headline font-bold text-primary">
          {t[titleKey]}
        </h1>
        <div className="text-muted-foreground">
          <span>{t.by} {article.author}</span> | <span>{article.date}</span>
        </div>
      </header>
