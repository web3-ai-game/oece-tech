...
import { useUserBBSStats } from '@/lib/supabase/bbs-stats-hooks';
import { signOut, generateInviteCode } from '@/lib/supabase/auth';
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card';
import { LayoutDashboard, UserCircle, KeyRound, MessageSquare, Bot, BookOpen, Star, Zap, ArrowRight, Copy, LogOut, Plus, Check, Sparkles, Shield, Globe, Eye, TrendingUp, FileText } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const { stats: bbsStats, loading: statsLoading } = useUserBBSStats(user?.id);
  ...
  return (
    <div className="space-y-6 p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto">
      ...
      {/* BBS统计卡片 */}
      <Card className="border-accent/20 bg-gradient-to-br from-accent/10 to-primary/5 backdrop-blur">
        <CardHeader>
        ...
        <CardContent>
          {statsLoading ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="p-4 border border-primary/20 rounded-lg bg-primary/5 hover:bg-primary/10 transition-colors">
                ...
                <p className="text-2xl font-bold font-mono text-primary">{bbsStats.posts_count}</p>
              </div>
              
              {/* 回复数 */}
              <div className="p-4 border border-accent/20 rounded-lg bg-accent/5 hover:bg-accent/10 transition-colors">
                <div className="flex items-center gap-2 mb-1">
                  <MessageSquare className="h-4 w-4 text-accent" />
                  <p className="text-xs text-muted-foreground font-mono">REPLIES</p>
                </div>
                <p className="text-2xl font-bold font-mono text-accent">{bbsStats.replies_count}</p>
              </div>
              
              {/* 总浏览 */}
              <div className="p-4 border border-blue-500/20 rounded-lg bg-blue-500/5 hover:bg-blue-500/10 transition-colors">
                <div className="flex items-center gap-2 mb-1">
                  <Eye className="h-4 w-4 text-blue-400" />
                  <p className="text-xs text-muted-foreground font-mono">VIEWS</p>
                </div>
                <p className="text-2xl font-bold font-mono text-blue-400">{bbsStats.total_views}</p>
              </div>
              
              {/* 收到回复 */}
              <div className="p-4 border border-green-500/20 rounded-lg bg-green-500/5 hover:bg-green-500/10 transition-colors">
                <div className="flex items-center gap-2 mb-1">
                  <TrendingUp className="h-4 w-4 text-green-400" />
                  <p className="text-xs text-muted-foreground font-mono">ENGAGEMENT</p>
                </div>
                <p className="text-2xl font-bold font-mono text-green-400">{bbsStats.total_replies_received}</p>
              </div>
            </div>
          )}
          ...
          <div className="flex gap-2 mt-4 pt-4 border-t border-border/50">
            <Link href="/bbs/new" className="flex-1">
              <Button variant="outline" size="sm" className="w-full gap-2 font-mono border-primary/30 hover:bg-primary/10">
                <Plus className="h-4 w-4" />
                NEW_POST
              </Button>
            </Link>
            <Link href="/bbs" className="flex-1">
              <Button variant="outline" size="sm" className="w-full gap-2 font-mono border-accent/30 hover:bg-accent/10">
