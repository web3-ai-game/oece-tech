...
import { Cpu, Mail, Lock, User, Eye, EyeOff, ArrowRight, Check } from 'lucide-react';
...
export default function RegisterPage() {
  ...
  const passwordStrength = () => {
    ...
    if (p.length < 6) return { level: 0, text: '太短', color: 'red' };
    if (p.length < 8) return { level: 1, text: '弱', color: 'orange' };
    if (/[A-Z]/.test(p) && /[0-9]/.test(p)) return { level: 3, text: '強', color: 'green' };
    return { level: 2, text: '中等', color: 'yellow' };
  };
    ...
    <div className="min-h-screen flex items-center justify-center px-6 py-12">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="text-2xl font-bold">創建帳戶</h1>
          <p className="text-gray-400 mt-2">開啟你的知識蒸餾之旅</p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              {form.password && (
                <div className="mt-2 flex items-center gap-2">
                  ...
                  <span className={`text-xs text-${strength.color}-400`}>{strength.text}</span>
                </div>
              )}
            </div>
            ...
            <div className="flex items-start gap-2 text-sm">
              ...
              <span className="text-gray-400">
                我同意{' '}
                <Link href="/terms" className="text-cyan-400 hover:text-cyan-300">服務條款</Link>
                {' '}和{' '}
                <Link href="/privacy" className="text-cyan-400 hover:text-cyan-300">隱私政策</Link>
              </span>
            </div>
            ...
            >
              {loading ? '註冊中...' : '創建帳戶'} <ArrowRight className="w-4 h-4" />
            </button>
          </form>
